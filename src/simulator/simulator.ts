...
let chat: string[] = [];
let variables: any = {};
let currentLinesIndex: number[] = [];

let outputChannel: vscode.OutputChannel | null = null;
let doc: vscode.TextDocument;

export function startSimulator(document: vscode.TextDocument, input: string) {
    doc = document;
...
function simulateInput(input: string, fromBot: boolean = false) {
    const lines = doc.getText().split('\n').map((text, index) => ({ text: text.trim(), index }));
    const triggeredLines: { text: string, index: number }[] = [];
...
function executeLines() {
    ...
    if (currentLinesIndex.length > 0) executeLines();
}

function populateVariables(text: string): string {
    return text.replace(/\{([a-zA-Z0-9]+?)\}/g, (match, variableName) => {
        return variables[variableName] || match;
    });
}

function executeLine(line: { text: string, index: number }): number | null {
    const { text, index } = line;

    if (text.startsWith('#')) return index + 1;
